...
- 格式一：ASCII
  ...
  - 字段说明：
    - n：样本序号（从 1 开始）
    - timestamp 为时间戳字段。当 .CFG 文件中的 nrates 和 samp 变量为非零时，该字段为非关键字段；当 nrates 和 samp 变量为零时，该字段为关键字段。该字段类型为整数数值型，最小长度为 1 个字符，最大长度为 13 个字符。时间的基本单位为微秒或纳秒，具体取决于 .CFG 文件中对日期/时间戳的定义。从数据文件中第一个数据采样点到由任一时间戳字段标记的采样点之间的经过时间，等于时间戳值与配置文件中时间倍率的乘积（timestamp × timemult）。当 nrates 和 samp 变量信息可用且同时也提供了时间戳信息时，为获得更精确的定时，应优先使用 nrates 和 samp 变量。
